...
#
# Tags look like `v<coturn version>-<release>`:
#
# - if defaults/main.yml points at a coturn version that has never been
...
# `coturn_version` is the literal value that Renovate bumps. Anchoring to the
# start of the line keeps this away from the variables derived from it
# (`coturn_container_image_tag` and friends), whose values are Jinja templates
# rather than a version.
version="$(sed -nE 's|^coturn_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the coturn version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `4.9.0`), but the tags
# do (`v4.9.0-0`). Stripping any `v` before prepending one keeps this correct
